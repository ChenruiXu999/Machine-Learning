...
vocab.set_default_index(vocab['<unk>'])#映射成数字index
...
def get_batch(source: Tensor, i: int) -> Tuple[Tensor, Tensor]:
    ...
    target = source[i+1:i+1+seq_len].reshape(-1)#y相当于x往后平移一位
    return data, target

ntokens = len(vocab)  # size of vocabulary 词库的总数量
emsize = 200  # embedding dimension 一个词被embedding成的纬度
...
scheduler = torch.optim.lr_scheduler.StepLR(optimizer, 1.0, gamma=0.95)#设置随着epoch的增大而逐渐减小学习率
